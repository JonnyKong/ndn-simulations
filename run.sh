# wifi range
rm adhoc-result-new-pattern/syncDuration-range-retx.txt
...
do
  ...
  NS_LOG='SyncForSleep' ./waf --run "sync-for-sleep-movepattern --pauseTime=0 --lossRate=0 --mobileNodeNum=10 --run=0 --wifiRange=$range --useHeartbeat=false --useBeacon=false --useRetx=true" > ${log_file_name} 2>&1
  result_file_name="adhoc-result-new-pattern/syncDuration-range-retx.txt"
...
# wifi range
rm adhoc-result-new-pattern/syncDuration-range-beacon.txt
range_list=(50 60 70 80 90 100 120 140 160)
range_index=0
while(( $range_index < 9))
do
  ...
  result_file_name="adhoc-result-new-pattern/syncDuration-range-beacon.txt"
  # result_file_name="adhoc-result/test.txt"
  python syncDuration.py ${log_file_name} 10 >> ${result_file_name}
  let "range_index++"
done
